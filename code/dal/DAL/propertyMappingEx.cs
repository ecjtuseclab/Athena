...
namespace Athena.dal
{
    public class propertyMappingEx : RepositoryBase<propertymapping>, IpropertyMappingEx
    {
        ...
        /// 表单提交
        /// </summary>
        /// <param name="pm">表单提交的一条数据字典数据</param>
        /// <param name="keyValue">主键</param>
        public void SubmitForm(propertymapping pm, string keyValue)
        {
            if (!string.IsNullOrEmpty(keyValue))
            {
                if (isLegalEntity(pm.propertyName,pm.propertyValue,pm.propertyMeaning,pm.remark)){
                    int id = Convert.ToInt32(keyValue);
                    pm.id = id;
                    db.Update<propertymapping>(pm);
                }
            }
            else
            {
                if (isLeagalEntityname(pm.propertyName))
                    db.Insert<propertymapping>(pm);
            }
        }
