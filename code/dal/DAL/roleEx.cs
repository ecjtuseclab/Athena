...
//模块及代码页功能描述：对role表的数据处理（艾美珍）
//最后修改时间:2018-01-25
//修改日志：
//2017-04-11 张婷婷接口方法添加、新增获取所有对象条数方法(张婷婷)
...
namespace Athena.dal
{
    public class roleEx : RepositoryBase<role>, IroleEx
    {
        ...
        /// <returns></returns>
        public override int insert(role ro)
        {
            try
            {
       ...
       /// <returns></returns>
        ...
        //{
        //    role ro = db.Queryable<role>().Where(r => r.id == id).FirstOrDefault();
        //    return delete(ro);
        //}
        /// <summary>
        /// 通过角色名获取部分角色信息
